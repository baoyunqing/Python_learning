...
df.to_csv('processed-tmdb-movies.csv')
...
df[df['popularity']>5]
df[(df['popularity'])>5&(df['release_date']>'1996')]
...
df.groupby('director')['popularity'].agg(['mean']).sort_values(by='mean', ascending=False)
...
for i in director_num:
    data = df.loc[df['director'] == i].nlargest(3, 'revenue_adj')
    analysis = analysis.append(data)

fig = plt.figure(figsize=(15, 10)) 
ax = sns.barplot(data=analysis, y='original_title', x='revenue_adj', hue='director',                 dodge=False)
plt.xlabel('revenue')
'''
分析:
史蒂芬·斯皮尔伯格拍的电影数量很多，票房是最高的
他擅长拍科幻片，惊悚片和动作片
票房高的电影大部分的都是好莱坞的大制作影片
'''
